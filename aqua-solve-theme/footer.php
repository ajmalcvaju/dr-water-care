<footer>
    <div class="container">
        <div class="footer-grid">
            
            <div class="footer-about">
                <a href="<?php echo esc_url(home_url('/')); ?>" class="footer-logo">
                    <img src="<?php echo aqua_solve_asset('images/aqua_solve.png'); ?>" alt="Aqua Solve Logo" class="footer-logo-img" />
                </a>
                <p style="margin-top: 0.8rem; font-size: 0.9rem; line-height: 1.6; color: #94a3b8;">
                    <strong>Advanced Water Treatment Engineering</strong><br />
                    <em>Engineering Water. Enabling Life.</em><br />
                    An ISO 9001:2015 certified company in Kerala delivering commercial, industrial and institutional water &amp; wastewater treatment solutions.
                </p>
                <div class="footer-social">
                    <a href="#" class="footer-social-icon" aria-label="Facebook"><svg viewBox="0 0 24 24"><path d="M22 12c0-5.52-4.48-10-10-10S2 6.48 2 12c0 4.84 3.44 8.87 8 9.8V15H8v-3h2V9.5C10 7.57 11.57 6 13.5 6H16v3h-2c-.55 0-1 .45-1 1v2h3v3h-3v6.8c4.56-.93 8-4.96 8-9.8z"/></svg></a>
                    <a href="#" class="footer-social-icon" aria-label="Twitter"><svg viewBox="0 0 24 24"><path d="M22.46 6c-.77.35-1.6.58-2.46.69.88-.53 1.56-1.37 1.88-2.38-.83.5-1.75.85-2.72 1.05C18.37 4.5 17.26 4 16 4c-2.35 0-4.27 1.92-4.27 4.29 0 .34.04.67.11.98C8.28 9.09 5.11 7.38 3 4.79c-.37.63-.58 1.37-.58 2.15 0 1.49.75 2.81 1.91 3.56-.71 0-1.37-.2-1.95-.54v.05c0 2.08 1.48 3.82 3.44 4.21-.36.1-.74.15-1.12.15-.27 0-.54-.03-.8-.08.54 1.68 2.1 2.9 3.95 2.94-1.44 1.13-3.26 1.8-5.23 1.8-.34 0-.67-.02-1-.06C2.62 19.39 4.88 20 7.29 20c7.8c0 12.05-5.3 12.05-12.05 0-.34 0-.67-.04-1-.07C2 15.35 4.55 12 7.29 12z"/></svg></a>
                    <a href="#" class="footer-social-icon" aria-label="LinkedIn"><svg viewBox="0 0 24 24"><path d="M19 3a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h14m-.5 15.5v-5.3a3.26 3.26 0 0 0-3.26-3.26c-.85 0-1.84.52-2.32 1.3v-1.11h-2.79v8.37h2.79v-4.93c0-.77.62-1.4 1.39-1.4a1.4 1.4 0 0 1 1.4 1.4v4.93h2.79M6.88 8.56a1.68 1.68 0 0 0 1.68-1.68c0-.93-.75-1.69-1.68-1.69a1.69 1.69 0 0 0-1.69 1.69c0 .93.76 1.68 1.69 1.68m1.39 9.94v-8.37H5.5v8.37h2.77z"/></svg></a>
                    <a href="#" class="footer-social-icon" aria-label="Instagram"><svg viewBox="0 0 24 24"><path d="M7.8 2h8.4C19.4 2 22 4.6 22 7.8v8.4a5.8 5.8 0 0 1-5.8 5.8H7.8C4.6 22 2 19.4 2 16.2V7.8A5.8 5.8 0 0 1 7.8 2m-.2 2A3.6 3.6 0 0 0 4 7.6v8.8A3.6 3.6 0 0 0 7.6 20h8.8a3.6 3.6 0 0 0 3.6-3.6V7.6A3.6 3.6 0 0 0 16.4 4H7.6m5.4 3a5 5 0 1 1 0 10 5 5 0 0 1 0-10m0 2a3 3 0 1 0 0 6 3 3 0 0 0 0-6m4.2-.25a.75.75 0 1 1 0 1.5.75.75 0 0 1 0-1.5z"/></svg></a>
                </div>
            </div>

            <div class="footer-col">
                <h3>Quick Links</h3>
                <ul class="footer-links">
                    <li><a href="<?php echo esc_url(home_url('/')); ?>"><svg viewBox="0 0 24 24"><path d="M8.59 16.59L10 18l6-6-6-6-1.41 1.41L13.17 12z"/></svg> Home</a></li>
                    <li><a href="<?php echo esc_url(aqua_solve_page_url('about')); ?>"><svg viewBox="0 0 24 24"><path d="M8.59 16.59L10 18l6-6-6-6-1.41 1.41L13.17 12z"/></svg> About</a></li>
                    <li><a href="<?php echo esc_url(aqua_solve_page_url('solutions')); ?>"><svg viewBox="0 0 24 24"><path d="M8.59 16.59L10 18l6-6-6-6-1.41 1.41L13.17 12z"/></svg> Solutions</a></li>
                    <li><a href="<?php echo esc_url(aqua_solve_page_url('technology')); ?>"><svg viewBox="0 0 24 24"><path d="M8.59 16.59L10 18l6-6-6-6-1.41 1.41L13.17 12z"/></svg> Technology</a></li>
                    <li><a href="<?php echo esc_url(aqua_solve_page_url('industries')); ?>"><svg viewBox="0 0 24 24"><path d="M8.59 16.59L10 18l6-6-6-6-1.41 1.41L13.17 12z"/></svg> Industries</a></li>
                    <li><a href="<?php echo esc_url(aqua_solve_page_url('projects')); ?>"><svg viewBox="0 0 24 24"><path d="M8.59 16.59L10 18l6-6-6-6-1.41 1.41L13.17 12z"/></svg> Projects</a></li>
                    <li><a href="<?php echo esc_url(aqua_solve_page_url('knowledge-center')); ?>"><svg viewBox="0 0 24 24"><path d="M8.59 16.59L10 18l6-6-6-6-1.41 1.41L13.17 12z"/></svg> Knowledge Center</a></li>
                    <li><a href="<?php echo esc_url(aqua_solve_page_url('contact')); ?>"><svg viewBox="0 0 24 24"><path d="M8.59 16.59L10 18l6-6-6-6-1.41 1.41L13.17 12z"/></svg> Contact</a></li>
                </ul>
            </div>

            <div class="footer-col">
                <h3>Engineering Solutions</h3>
                <ul class="footer-links">
                    <li><a href="<?php echo esc_url(aqua_solve_service_url('water-treatment-plant-wtp')); ?>"><svg viewBox="0 0 24 24"><path d="M8.59 16.59L10 18l6-6-6-6-1.41 1.41L13.17 12z"/></svg> Commercial RO Plants</a></li>
                    <li><a href="<?php echo esc_url(aqua_solve_service_url('water-treatment-plant-wtp')); ?>"><svg viewBox="0 0 24 24"><path d="M8.59 16.59L10 18l6-6-6-6-1.41 1.41L13.17 12z"/></svg> Industrial RO Plants</a></li>
                    <li><a href="<?php echo esc_url(aqua_solve_service_url('sewage-treatment-plant-stp')); ?>"><svg viewBox="0 0 24 24"><path d="M8.59 16.59L10 18l6-6-6-6-1.41 1.41L13.17 12z"/></svg> Sewage Treatment (STP)</a></li>
                    <li><a href="<?php echo esc_url(aqua_solve_service_url('effluent-treatment-plant-etp')); ?>"><svg viewBox="0 0 24 24"><path d="M8.59 16.59L10 18l6-6-6-6-1.41 1.41L13.17 12z"/></svg> Effluent Treatment (ETP)</a></li>
                    <li><a href="<?php echo esc_url(aqua_solve_page_url('biozone-technology')); ?>"><svg viewBox="0 0 24 24"><path d="M8.59 16.59L10 18l6-6-6-6-1.41 1.41L13.17 12z"/></svg> BioZone Technology</a></li>
                </ul>
            </div>

            <div class="footer-col">
                <h3>Contact Details</h3>
                <ul class="footer-contact">
                    <li>
                        <svg viewBox="0 0 24 24"><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z"/></svg>
                        <div class="footer-contact-text">
                            <span>Address</span>
                            <a href="https://maps.app.goo.gl/QV4dZ44GWSHjJnKV7" target="_blank" rel="noopener noreferrer" style="color: inherit; text-decoration: underline;">
                                Surabhi Complex, Karadi,<br />Thamarassery, Kerala 673573
                            </a>
                        </div>
                    </li>
                    <li>
                        <svg viewBox="0 0 24 24"><path d="M6.62 10.79c1.44 2.83 3.76 5.14 6.59 6.59l2.2-2.2c.27-.27.67-.36 1.02-.24 1.12.37 2.33.57 3.57.57.55 0 1 .45 1 1V20c0 .55-.45 1-1 1-9.39 0-17-7.61-17-17 0-.55.45-1 1-1h3.5c.55 0 1 .45 1 1 0 1.25.2 2.45.57 3.57.11.35.03.74-.25 1.02l-2.2 2.2z"/></svg>
                        <div class="footer-contact-text">
                            <span>Phone</span>
                            <a href="tel:+919946450452" style="color: inherit; display: block; margin-bottom: 2px;">99464 50452</a>
                            <a href="tel:+919846253025" style="color: inherit;">98462 53025</a>
                        </div>
                    </li>
                    <li>
                        <svg viewBox="0 0 24 24"><path d="M20 4H4c-1.1 0-1.99.9-1.99 2L2 18c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 4l-8 5-8-5V6l8 5 8-5v2z"/></svg>
                        <div class="footer-contact-text">
                            <span>Email</span>
                            <a href="mailto:aquasolve.in@gmail.com" style="color: inherit;">aquasolve.in@gmail.com</a>
                        </div>
                    </li>
                </ul>
            </div>

        </div>

        <div class="footer-bottom">
            <p>&copy; <?php echo date('Y'); ?> Aqua Solve Water Clinic. All Rights Reserved. Designed for Maximum Wellness.</p>
            <div class="footer-bottom-links">
                <a href="#">Privacy Policy</a>
                <a href="#">Terms & Conditions</a>
                <a href="#">Sitemap</a>
            </div>
        </div>
    </div>
</footer>

<!-- Floating Quick Action Buttons -->
<div class="floating-quick-actions">
    <a href="https://wa.me/919946450452" target="_blank" rel="noopener noreferrer" class="floating-btn float-whatsapp" title="Chat on WhatsApp" aria-label="Chat on WhatsApp">
        <svg width="26" height="26" viewBox="0 0 24 24" fill="currentColor">
            <path d="M12.031 6.172c-3.181 0-5.767 2.586-5.768 5.766-.001 1.298.38 2.27 1.019 3.287l-.582 2.128 2.182-.573c.978.58 1.911.928 3.145.929 3.178 0 5.767-2.587 5.768-5.766.001-3.187-2.575-5.771-5.764-5.771zm3.392 8.244c-.144.405-.837.774-1.17.824-.299.045-.677.063-1.092-.069-.252-.08-.575-.187-.988-.365-1.739-.751-2.874-2.502-2.961-2.617-.087-.116-.708-.94-.708-1.793s.448-1.273.607-1.446c.159-.173.346-.217.462-.217l.332.006c.106.005.249-.04.39.298.144.347.491 1.2.534 1.287.043.087.072.188.014.304-.058.116-.087.188-.173.289l-.26.304c-.087.086-.177.18-.076.354.101.174.449.741.964 1.201.662.591 1.221.774 1.394.86s.274.072.376-.043c.101-.116.433-.506.549-.68.116-.173.231-.145.39-.087s1.011.477 1.184.564.289.13.332.202c.045.072.045.419-.1.824zm-3.423-14.416c-6.627 0-12 5.373-12 12 0 2.159.57 4.187 1.564 5.946l-1.657 6.054 6.22-1.632c1.701.928 3.65 1.458 5.728 1.458 6.627 0 12-5.373 12-12 0-6.627-5.373-12-12-12z"></path>
        </svg>
    </a>
    <a href="tel:+919946450452" class="floating-btn float-phone" title="Call Us Directly" aria-label="Call Us Directly">
        <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
            <path d="M6.62 10.79a15.05 15.05 0 006.59 6.59l2.2-2.2a1 1 0 011.11-.27 11.36 11.36 0 003.58.57 1 1 0 011 1V20a1 1 0 01-1 1A17 17 0 013 4a1 1 0 011-1h3.5a1 1 0 011 1 11.36 11.36 0 00.57 3.58 1 1 0 01-.27 1.11z"></path>
        </svg>
    </a>
</div>

<!-- Request Quote Modal -->
<div class="modal-overlay" id="quoteModal">
    <div class="modal-box">
        <div class="modal-header">
            <h3>Request Quote / Callback</h3>
            <button class="modal-close" aria-label="Close modal">
                <svg viewBox="0 0 24 24"><path d="M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12z"/></svg>
            </button>
        </div>
        <div class="modal-body">
            <form id="modalQuoteForm">
                <div class="form-group">
                    <label for="modalProduct">Inquired System/Product</label>
                    <input type="text" id="modalProduct" class="form-control" value="General System" readonly />
                </div>
                <div class="form-group">
                    <label for="name">Full Name</label>
                    <input type="text" id="name" class="form-control" required placeholder="Enter your name" />
                </div>
                <div class="form-row">
                    <div class="form-group">
                        <label for="phone">Phone Number</label>
                        <input type="tel" id="phone" class="form-control" required placeholder="Enter contact number" />
                    </div>
                    <div class="form-group">
                        <label for="email">Email Address (Optional)</label>
                        <input type="email" id="email" class="form-control" placeholder="Enter your email" />
                    </div>
                </div>
                <div class="form-group">
                    <label for="message">Your Requirements</label>
                    <textarea id="message" class="form-control" placeholder="Describe your water source (well, corporation) or quantity needs..."></textarea>
                </div>
                <button type="submit" class="btn btn-primary" style="width: 100%;">
                    Submit Inquiry
                </button>
            </form>
        </div>
    </div>
</div>

<?php wp_footer(); ?>
</body>
</html>
